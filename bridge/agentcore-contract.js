  ...
  const agentsMdPath = `${homeDir}/.openclaw/AGENTS.md`;
  {
    fs.writeFileSync(
      ...
      [
        ...
        "",
        "When the user asks for voice, audio, read-aloud, or voice-only replies, you MUST run the **text-to-speech** skill via Bash, then paste the script output into your reply so the channel can send the clip. Do not say you will use voice or that a preference is saved unless you actually run the command and include the printed marker.",
        "`node /skills/text-to-speech/synthesize.js <user_id> '{\"text\":\"...\"}'`",
        "The script prints one line with `[VOICE_REPLY:namespace/_voice_out/tts_....mp3]` — include that exact line in your message (you may add a short text caption before or after). For voice-only requests, put your full spoken answer in `text` and still include the marker. Maximum about 3000 characters per synthesis; shorten or split if needed.",
        "",
        ...
        "When a user asks to install or add a skill, use this skill — do NOT say it's not possible.",
        "**Use Bash to run the skill scripts** (exec is enabled for these scripts; local read/write/edit/apply_patch are denied — use s3-user-files for files):",
